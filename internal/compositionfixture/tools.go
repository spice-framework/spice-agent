...
func newFixtureTool(name string, effect tool.Effect, replaySafety tool.ReplaySafety) (*fixtureTool, error) {
...
func NewReadTool(log *CleanupLog) (ToolAlias, lifecycle.Cleanup, error) {
	implementation, err := newFixtureTool("read", tool.EffectReadOnly, tool.ReplaySafe)
	if err != nil {
		return nil, nil, err
	}
...
func NewWriteTool(
	...
	implementation, err := newFixtureTool("write", tool.EffectMutating, tool.ReplayUnsafe)
	if err != nil {
		return nil, nil, err
	}
	return implementation, func(context.Context) error {

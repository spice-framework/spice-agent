...
| One synchronization boundary | The host lock covers stopping state, degradation reasons, active reservations, and configured limits. Immutable definitions are captured at that same boundary. |
...
| Determinism and concurrency | Degradation reasons remain sorted and repeated concurrent readers observe the same definitions, health state, limits, and reasons under the race detector. |

...
| Idempotent mutations | Owner/waiter races prove stable-client operation identity, canonical digest conflicts, committed-result precedence, independent waiter cancellation, per-client fairness, total bounds, and exact duplicate outcomes. An exact `AbandonOperation` result proves pre-commit failure, removes the unused identity and capacity, wakes duplicates, and admits one iterative retry owner; canceled waiters retain their context cause. Arbitrary, wrapped, nil-cause, result-bearing, and panicking failures still commit one bounded secret-safe uncertain outcome. |

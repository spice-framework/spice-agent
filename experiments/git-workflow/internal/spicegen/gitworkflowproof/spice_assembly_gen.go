...
func NewApplicationWithOptions(ctx context.Context, options ApplicationOptions) (*Application, error) {
	if ctx == nil {
		return nil, fmt.Errorf("construct application gitworkflowproof: context is nil")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("construct configuration schema for application gitworkflowproof: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resolve configuration for application gitworkflowproof: %w", err)
	}
	application.shutdownTimeout, err = configurationSnapshot.Duration("spice.shutdown-timeout")
	if err != nil {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application gitworkflowproof: %w", err))
	}
	if application.shutdownTimeout <= 0 {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application gitworkflowproof: duration must be positive"))
	}
	...
	application.components = Components{
		GitBackend:        dependencies.gitBackend,
		Provider1:         dependencies.gitInspect,
		GitAuthorityStore: dependencies.gitAuthorityStore,
		Provider3:         dependencies.gitCommit_staged,
		GitCommitGuard:    dependencies.gitCommitGuard,

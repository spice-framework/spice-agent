...
func NewApplicationWithOptions(ctx context.Context, options ApplicationOptions) (*Application, error) {
	if ctx == nil {
		return nil, fmt.Errorf("construct application twoworkerproof: context is nil")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("construct configuration schema for application twoworkerproof: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resolve configuration for application twoworkerproof: %w", err)
	}
	application.shutdownTimeout, err = configurationSnapshot.Duration("spice.shutdown-timeout")
	if err != nil {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application twoworkerproof: %w", err))
	}
	if application.shutdownTimeout <= 0 {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application twoworkerproof: duration must be positive"))
	}
	...
	application.components = Components{
		WorkerSession: dependencies.workerSession,
		Provider1:     dependencies.workerDelegate,

...
func NewApplicationWithOptions(ctx context.Context, options ApplicationOptions) (*Application, error) {
	if ctx == nil {
		return nil, fmt.Errorf("construct application sqliterecoveryproof: context is nil")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("construct configuration schema for application sqliterecoveryproof: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resolve configuration for application sqliterecoveryproof: %w", err)
	}
	application.shutdownTimeout, err = configurationSnapshot.Duration("spice.shutdown-timeout")
	if err != nil {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application sqliterecoveryproof: %w", err))
	}
	if application.shutdownTimeout <= 0 {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application sqliterecoveryproof: duration must be positive"))
	}
	...
	application.components = Components{
		SqliteRecoveryOptions:      dependencies.sqliteRecoveryOptions,
		SqliteRecoveryStoreFactory: dependencies.sqliteRecoveryStoreFactory,
		SqliteRecoveryProof:        dependencies.sqliteRecoveryProof,
	}

...
func NewApplicationWithOptions(ctx context.Context, options ApplicationOptions) (*Application, error) {
	if ctx == nil {
		return nil, fmt.Errorf("construct application planningproof: context is nil")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("construct configuration schema for application planningproof: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("resolve configuration for application planningproof: %w", err)
	}
	application.shutdownTimeout, err = configurationSnapshot.Duration("spice.shutdown-timeout")
	if err != nil {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application planningproof: %w", err))
	}
	if application.shutdownTimeout <= 0 {
		return nil, application.coordinator.Abort(ctx, fmt.Errorf("decode shutdown timeout for application planningproof: duration must be positive"))
	}
	...
	application.components = Components{
		Provider0:               dependencies.planningDefault,
		PlanningProofProvider:   dependencies.planningProofProvider,
		PlanningProofDispatcher: dependencies.planningProofDispatcher,
		PlanningProofEngine:     dependencies.planningProofEngine,

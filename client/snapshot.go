...
func ParseSnapshot(encoded []byte) (Snapshot, error) {
	if len(encoded) == 0 || len(encoded) > MaximumSnapshotBytes {
		return Snapshot{}, fmt.Errorf("client snapshot must be between 1 and %d bytes", MaximumSnapshotBytes)
	}
...
func (snapshot Snapshot) MarshalBinary() ([]byte, error) {
	if len(snapshot.encoded) == 0 || len(snapshot.encoded) > MaximumSnapshotBytes {
